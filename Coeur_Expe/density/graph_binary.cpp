...
Graph::Graph(char *filename, char *filename_w, int type) {
  ...
  if (type==WEIGHTED) {
    ...
    finput_w.read((char *)&weights[0], (long)nb_links*sizeof(int));  
  }    
...
void
Graph::densite_weighted(string filename) {
	ifstream finput;
	unsigned int node, pos, pos_end;

	finput.open(filename.c_str(), fstream::in);
	node = 0;

	vector<bool> isInCc(nb_nodes, false);
	vector<unsigned int> nodeInCc;
	...
	while (finput >> node) {
		//if (node > nb_nodes)
		//	cout << node << " " << nb_nodes << endl;
		isInCc[node] = true;
		nodeInCc.push_back(node);

	}
	finput.close();
	
	cerr << nodeInCc.size() << endl;
	
	float m_plus=0, c_plus=0;
	unsigned int nb_links_in =0, nb_links_out=0; 
	for (unsigned i =0; i < nodeInCc.size(); i++) {
		node = nodeInCc[i];
		if(node == 0) pos = 0; else pos = degrees[node - 1];
		pos_end = degrees[node];
		for (unsigned int j = pos; j < pos_end; j++) {
			//cout << "(" << node << ", " << links[j] << ")" << endl;
			if (isInCc[links[j]]) {
				//m_plus+=weights[j];
				m_plus++;
				nb_links_in++;
	...
	//cout << c_plus << endl;
	//DENSITY
	cout << ( 2 * m_plus / ( nodeInCc.size() * (nodeInCc.size() - 1) ) ) << "\t";
	//CUT RATIO
	cout << c_plus / ( nodeInCc.size() * (nb_nodes - nodeInCc.size()) )<< "\t";
	//CONDUCTANCE
	cout << c_plus / ( m_plus + c_plus)<< endl;
