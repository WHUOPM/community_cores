...
int main(int argc, char* argv[]) {
	...
	if ( argc > 3 ) {
		...
		ofstream hugeGraph;
		hugeGraph.open((path + "Cooccur_"+graphName).c_str(), ios::out);
		...
		ofstream file1(path + "RUN/" + argv[3] + "/1/Cooccur_" + graphName);
		ofstream file2(path + "RUN/" + argv[3] + "/2/Cooccur_" + graphName);
		ofstream file3(path + "RUN/" + argv[3] + "/3/Cooccur_" + graphName);
		ofstream file4(path + "RUN/" + argv[3] + "/4/Cooccur_" + graphName);
		ofstream file5(path + "RUN/" + argv[3] + "/5/Cooccur_" + graphName);
		ofstream file6(path + "RUN/" + argv[3] + "/6/Cooccur_" + graphName);
		ofstream file7(path + "RUN/" + argv[3] + "/7/Cooccur_" + graphName);
		ofstream file8(path + "RUN/" + argv[3] + "/8/Cooccur_" + graphName);
		ofstream file9(path + "RUN/" + argv[3] + "/9/Cooccur_" + graphName);

		while ( inFile >> source >> target >> nbOccur ) {
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.1)) ) {

				file1 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.2)) ) {

				file2 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.3)) ) {

				file3 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.4)) ) {

				file4 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.5)) ) {

				file5 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.6)) ) {

				file6 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.7)) ) {

				file7 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.8)) ) {

				file8 << source << " " << target << endl;

			}
			if( (double) (((double)nbOccur) / atoi(argv[2]) >= (0.9))) {
		...
		for ( unsigned int i = 1; i < 10; i++ ) {
			...
			Digraph d = Digraph();
			d.loadEdgeList(path + "RUN/" + argv[3] + "/" + to_string(i) + "/Cooccur_" + graphName, false);
			...
			ofstream unweightedRelabeledGraph(path + "RUN/" + argv[3] + "/" + to_string(i) + "/Cooccur_" + graphName +"_relabeled", ios::out);
			ifstream inFile(path + "Cooccur_"+graphName);

			while ( inFile >> source >> target >> weight ) {

				// INUTILE : il suffit de reparcourir le Cooccur_ qui est dans le dossier alpha
				if ( (double) (((double)weight) / atoi(argv[2]) >= (i/10.0)) ) {
